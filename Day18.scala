...
object Day18 extends App {
  ...
  case class Branch(left: Tree, right: Tree, override val id: Int) extends Tree(id) {
  ...
  object Tree {
      ...
      if (tree == oldValue) newValue
        ...
        tree match {
          ...
          case b: Branch =>
            ...
            b.copy(left = leftSubTree, right = rightSubTree)
        }
      }
    }
    ...
    def fromString(str: String): Tree = {
      ...
      def loop: Tree = {
        ...
        currChar match {
          ...
          case c =>
            val charsUntilCommaOrClosingBrace = chars.takeWhile(c => !(Set('[', ',') contains c))
    ...
    def split(tree: Tree): Tree = {
      ...
      val treeUpdatedO = findLeafToSplit(tree).map { case leaf @ Leaf(value, id) =>
        val left = value / 2
        val right = value - left
        val newBranch = Branch(Leaf(left, getBranchId), Leaf(right, getBranchId), getBranchId)
    ...
    def explode(tree: Tree): Tree = {
      ...
      def findBranchToExplode(tree: Tree, parents: List[Branch]): Option[Result] = {
        tree match {
          case l: Leaf => None
          case branch: Branch =>
            if (parents.size == 4) Some(Result(branch, parents))
            else {
              findBranchToExplode(branch.left, parents.prepended(branch)) match {
                case Some(result) => Some(result)
                case None => findBranchToExplode(branch.right, parents.prepended(branch))  
              } 
            }  
        }
      }
      ...
      val branchToExplodeO: Option[Result] = findBranchToExplode(tree, List.empty[Branch])
      ...
      val treeUpdatedO: Option[Tree] =
        branchToExplodeO.map { case Result(branch, parents) =>
          val leftLeafO: Option[Leaf] = {
            ...
            while (currParents.headOption.map(_.left.id) == Some(currBranch.id)) {
              currBranch = currParents.head
              currParents = currParents.tail
            }
            
            currParents.headOption.map { parent => getRightmostLeaf(parent.left) }
          }

          val rightLeafO: Option[Leaf] = {
            var currBranch: Branch = branch
            var currParents: List[Branch] = parents
            while (currParents.headOption.map(_.right.id) == Some(currBranch.id)) {
              currBranch = currParents.head
              currParents = currParents.tail
            }
            
            currParents.headOption.map { parent => getLeftmostLeaf(parent.right) }
          }
