...
object Day23B extends App {
  case class Position(row: Int, col: Int) {
    def depth: Int =
      row match {
        ...
        case 4 => 3 // pos3 in Room
        case 5 => 4 // pos4 in Room
      }
  }
  object Position {
  ...
  object Room {
    val ROOM_A: Room =
      Room(Position(2, 3), Position(3, 3), Position(4, 3), Position(5, 3))
    val ROOM_B: Room =
      Room(Position(2, 5), Position(3, 5), Position(4, 5), Position(5, 5))
    val ROOM_C: Room =
      Room(Position(2, 7), Position(3, 7), Position(4, 7), Position(5, 7))
    val ROOM_D: Room =
      Room(Position(2, 9), Position(3, 9), Position(4, 9), Position(5, 9))
  }
  ...
  case class Pod(cat: Category, energy: Int, pos: Position, destRoom: Room) {
    def isHome: Boolean =
      (destRoom.pos1 == pos || destRoom.pos2 == pos || destRoom.pos3 == pos || destRoom.pos4 == pos)
  ...
  case class State(
    ...
    def nextStates: Map[State, Long] = {
      ...
      for (currPod <- pods) {
        ...
        if (currPod.depth == 0) {
          // if destRooms first position is empty & rest of positions are either empty or occupied by other pods of same type, proceed
          val firsPosIsEmpty: Boolean = {
            openSpaces.contains(OpenSpace(destRoom.pos1))
          }

          val otherPositionsAreNotOccupiedByDiffPod: Boolean = {
            var result = true
            for {
              pos <- List(destRoom.pos2, destRoom.pos3, destRoom.pos4)
            } {
              pods.find(_.pos == pos).map { pod =>
                if (pod.cat != currPod.cat)
                  result = false
              }
            }
          ...
          if (firsPosIsEmpty && otherPositionsAreNotOccupiedByDiffPod) {
        ...
        } else if (currPod.depth != 4) {
          // it makes sense to go down if there is no other pod there and this is the home room
          if (currPod.isHome) {
            val currPos = currPod.pos
            val deeperPos = currPos.copy(row = currPos.row + 1)
            if (Position.isPathClear(openSpaces, currPod.pos, deeperPos))
              addNewState(currPod, deeperPos)
          }
          ...
          // if the pod is home in the last position, don't move it
          if (!currPod.isHome) {
            // going to the hallway
            // open spaces in front of other rooms is not an option
            val eligibleHallwayOpenSpaces =
  ...
  val inputState: State = {
    val input: List[List[Char]] =
      readAllLines("day-23B-input.txt")
        .map(_.toList)
